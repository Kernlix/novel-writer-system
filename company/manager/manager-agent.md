...
- **新会话时**：执行 `company/manager/hooks/session-init.md` 初始化会话上下文
- **会话结束时**：执行 `company/manager/hooks/session-end.md` 压缩上下文、清理临时数据
- **创作讨论前**：执行 `company/manager/hooks/pre-discuss.md` 汇总当前状态
...
| Hook | 用途 |
|:-----|:------|
| `pre-archive` | 归档前——备份当前状态、记录Agent/Skill数量 |
| `post-archive` | 归档后——校验归档完整性、清理临时文件 |
